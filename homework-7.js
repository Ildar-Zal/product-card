...
const newSocialComments = socialComments.map((socialComment) => {
  socialComment.postId = (socialComment.id <= 5) ? 2 : 1;
...
const socialCommentsWithInvalid = socialComments.map(socialComment => {
  const isInvalid = socialComment.body.length > 180;
  return {...socialComment,isInvalid};
})
